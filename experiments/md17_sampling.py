...
if config.eval.model == 'GeoTDM':
    # Try to find the training config file automatically
    import glob
    if cond:
        # Look for any *_train_cond.yaml file in the training output directory
        yaml_pattern = os.path.join(train_output_path, '*_train_cond.yaml')
        yaml_files = glob.glob(yaml_pattern)
        if len(yaml_files) == 0:
            raise FileNotFoundError(f"No training config file found matching pattern: {yaml_pattern}")
        elif len(yaml_files) > 1:
            print(f"Warning: Multiple training config files found: {yaml_files}. Using the first one.")
        train_yaml_file = yaml_files[0]
    else:
        # Look for any *_train_uncond.yaml file
        yaml_pattern = os.path.join(train_output_path, '*_train_uncond.yaml')
        yaml_files = glob.glob(yaml_pattern)
        if len(yaml_files) == 0:
            raise FileNotFoundError(f"No training config file found matching pattern: {yaml_pattern}")
        elif len(yaml_files) > 1:
            print(f"Warning: Multiple training config files found: {yaml_files}. Using the first one.")
        train_yaml_file = yaml_files[0]
    print(f'Using training config file: {train_yaml_file}')
else:
